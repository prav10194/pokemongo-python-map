...
def main():
    ...
    counterCells=int(input('Enter number of cells to search : '))
    counter=1
    ...
    while True:
        ...
        for cell in h.cells:
            if cell.NearbyPokemon:
                ...
                for poke in cell.NearbyPokemon: #nearby different than visible
                    print('    (%s) %s' % (poke.PokedexNumber, pokemons[poke.PokedexNumber - 1]['Name']))
                    #print(other)
                    coord=str(other).split()[1]
                    latitude=str(coord).split(',')[0]
                    longitude=str(coord).split(',')[1]
                    #print(latitude,longitude)
                    gps_markers.append([pokemons[poke.PokedexNumber - 1], float(latitude),float(longitude)])
        ...
        for poke in visible:
            ...
            difflng = diff.lng().degrees
            direction = (('N' if difflat >= 0 else 'S') if abs(difflat) > 1e-4 else '')  + (('E' if difflng >= 0 else 'W') if abs(difflng) > 1e-4 else '')
            gps_markers.append([pokemons[poke.pokemon.PokemonId - 1], poke.Latitude, poke.Longitude])
        ...
        next = LatLng.from_point(Cell(CellId(walk[2])).get_center())
            ...
            #break
        #time.sleep(5)
        if counter<=counterCells:
            mapdraw()
            counter=counter+1
            #print("Counters: "+str(counter)+"\t"+str(counterCells))
            #time.sleep(5)
...
def mapdraw():
    file=open('remove-pokemons.csv')
    names=csv.reader(file)
    namesList=list(names)
    gmap = gmplot.GoogleMapPlotter(float(gps_markers[0][1]), float(gps_markers[0][2]), 16)
    #print(gps_markers)
    for a in gps_markers:
        #print(long(a[0]['Number']))
        for i in range(0,len(namesList[0])):
            if a[0]['Name'] not in namesList[0]:
                gmap.marker(float(a[1]), float(a[2]), "#"+str(long(a[0]['Number'])))
    gmap.draw("nearby.html")
